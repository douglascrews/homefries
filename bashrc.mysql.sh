...
"; \
	...
	${ECHODO} mysql --host=${1:-${MYSQL_HOST:-${default_host}}} --port=${2:-${MYSQL_PORT:-${default_port}}} --user=${3:-${MYSQL_USER:-${default_user}}} --password${password_equal}${4:-${MYSQL_PW}} --database=${5:-${MYSQL_DATABASE:-${default_database}}} --protocol=${6:-${MYSQL_PROTOCOL:-${default_protocol}}} --tee=mysql.log --comments=${7:-${MYSQL_COMMENTS:-${default_comments}}} --no-auto-rehash
}
...
\t MYSQL_PW (via prompt)\n \
\t MYSQL_DATABASE (default ${MYSQL_DATABASE:-${default_database}})\n \
...
"; \
	...
	if [[ "${ECHODO}" == "echodo" ]]; then
		echo "mysql --host=${MYSQL_HOST:-${default_host}} --port=${MYSQL_PORT:-${default_port}} --user=${MYSQL_USER:-${default_user}} --password${password_equal}\${MYSQL_PW} --database=${MYSQL_DATABASE:-${default_database}} --protocol=${MYSQL_PROTOCOL:-${default_protocol}} --tee=mysql.log --comments=${MYSQL_COMMENTS:-${default_comments}} --database=${MYSQL_DATABASE:-${default_database}} --column-names --silent --table --execute=\"${*}\""
	fi
	if [[ "${ECHODO}" == "echodont" ]]; then
		echodont "mysql --host=${MYSQL_HOST:-${default_host}} --port=${MYSQL_PORT:-${default_port}} --user=${MYSQL_USER:-${default_user}} --password${password_equal}\${MYSQL_PW} --database=${MYSQL_DATABASE:-${default_database}} --protocol=${MYSQL_PROTOCOL:-${default_protocol}} --tee=mysql.log --comments=${MYSQL_COMMENTS:-${default_comments}} ${MYSQL_DATABASE:-${default_database}} --execute=\"${*}\""
	else
		mysql --host=${MYSQL_HOST:-${default_host}} --port=${MYSQL_PORT:-${default_port}} --user=${MYSQL_USER:-${default_user}} --password${password_equal}${MYSQL_PW} --database=${MYSQL_DATABASE:-${default_database}} --protocol=${MYSQL_PROTOCOL:-${default_protocol}} --tee=mysql.log --comments=${MYSQL_COMMENTS:-${default_comments}} --database=${MYSQL_DATABASE:-${default_database}} --column-names --silent --table --execute="${*}"
	fi
}
export -f mysql_run
